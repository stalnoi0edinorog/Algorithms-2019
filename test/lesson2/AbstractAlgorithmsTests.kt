...
abstract class AbstractAlgorithmsTests {
    ...
    fun josephTask(josephTask: (Int, Int) -> Int) {
        ...
        assertEquals(480, josephTask(500, 100))
        assertEquals(36, josephTask(50, 10))
        assertEquals(28, josephTask(40, 3))
        Assertions.assertThrows(IllegalArgumentException::class.java) { josephTask(-8, 5) }
        Assertions.assertThrows(IllegalArgumentException::class.java) { josephTask(0, 5) }
        Assertions.assertThrows(IllegalArgumentException::class.java) { josephTask(0, 0) }
        Assertions.assertThrows(IllegalArgumentException::class.java) { josephTask(5, 0) }
        Assertions.assertThrows(IllegalArgumentException::class.java) { josephTask(8, -5) }
        var menNumber = 2
        for (i in 1..20) {
